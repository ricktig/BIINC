...
<body>
	...
	if(isset($_SESSION['user']) && 	in_array($_SESSION['user'],$adminusers) || in_array($_SESSION['user'],$dapdadminusers))
	{
		...
		{
				$pkid = mysql_real_escape_string($_POST['hiddenid']);
		
				$query = "UPDATE tblUsers SET Active='0' WHERE pkId = '" . $pkid . "'";
				...
				{
					echo '<h2>Officer Successfully Deleted</h2>';
					echo '<p>Officer ' . $_POST['firstname'] . ' ' . $_POST['lastname'] . ' was successfully deleted.</p>';
				}//end each record in dataset
				else
				{ //no voucher records found
					echo '<h2>Officer Deletion Failed</h2><br/>';
	...
	?>
		...
		<h2 class="center" style="margin:20px 0 0 0">Only authorized users can access the officer deletion form.</h2>
