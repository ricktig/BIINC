...
<body>
	...
	<?php include 'header.php';//display logo on logged in and not logged in page views?>
		...
		if (!$errorflag)
		{
			...
			$stmt = $db->prepare($sql);
			$stmt->execute(array('sha256password1'=>$sha256password1, 'mysalt'=>$mySalt,'username'=>$_POST['username']));
			
			$passwordresetok=$stmt->rowCount();
					
			//check for successfull password reset
			if($passwordresetok!=0)
			{
